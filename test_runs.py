...
def stat_runs(param_config_file, scenario_config_file, base_model_filepath, schedule_filepath, out_csv, out_img, strategies, num_deliveries_runs, availability_percents, num_stat_runs, record_output, generate_schedules):
    ...
    g = create_graph_from_file(graph_filename)
    	...
    	for availability_percent in availability_percents:
		    ...
		    if generate_schedules:
			    ...
			    for stat_run in stat_runs:
			    	...
			    	avails, variances = generate_window_base_availability_models_with_bernoulli_variance(node_requests[stat_run], start_time, availability_percent, params['budget'], params['time_interval'], params['availability_length'])
					...
					## true schedules
					true_schedules.append(sample_schedule_from_model(node_requests[stat_run], sampled_avails, mu, params['num_intervals']))
			    ...
			    true_schedules = []
				for stat_run in stat_runs:
					base_avails, base_variances, requests = load_base_models_from_file(base_model_filepath, num_stat_runs, num_deliveries, availability_percent, stat_run)
					true_avails, schedules = load_schedules_from_file(schedule_filepath, node_requests, num_stat_runs, num_deliveries, availability_percent, stat_run)
