...
generate_plots(strategies, num_deliveries_runs, availability_percents, csv_filename, plotting_mode='cr'):
	...
	with open(csv_filename) as csvfile:
		...
		for row in reader:
			...
			# last_deliver_time_ave = float(row[6])
			# last_deliver_time_stdev = float(row[7])
			total_maintenance_profit_ave = float(row[6])
			total_maintenance_profit_stdev = float(row[7])
			# total_profit_ave = float(row[8])
			# total_profit_stdev = float(row[9])
			competitive_ratio_ave = float(row[8])
			competitive_ratio_stdev = float(row[9])
			noise_amp = float(row[10])

			if num_deliveries in num_deliveries_runs:
				maintenance_crs[str(num_deliveries)][strategy][str(availability_percent)].append(maintenance_cr)
				competitive_ratios[str(num_deliveries)][strategy][str(availability_percent)].append(competitive_ratio)
			...
			for strategy in strategies:
					...
					competitive_ratio_stats[str(num_deliveries)][strategy] = {}
						for availability_percent in availability_percents:
							...
							maintenance_cr_stats[str(num_deliveries)][strategy].append([maintenance_cr_ave, maintenance_cr_stdev])
							competitive_ratio_stats[str(num_deliveries)][strategy].append([competitive_ratio_ave, competitive_ratio_stdev])
