...
def generate_simple_models(node_requests, start_time, availability_percent, budget, time_interval, availability_length, availability_chance):
    ...
    for request in node_requests:
        ...
        variances = []
        t = start_time
        num_intervals = int((budget - start_time)/time_intervals)
        ...
        model_variances[request] = variance
    ...
    for request in node_requests:
        ...
        for i in range(len(base_availability_models[request])):
            ...
            if sampling_method == 'gauss':
                new_avail = max(min(random.gauss(old_avail, math.sqrt(variance)), 0.99), 0.01)
...
### Generate schedule from "true" model
def generate_schedule(node_requests, availability_models, mu, num_intervals, schedule_generation_method, incorporate_observation):
    if schedule_generation_method == 'threshold':
        return generate_schedule_from_model_windows(node_requests, availability_models, mu, num_intervals)
    elif schedule_generation_method == 'temporal_consistency':
        return sample_schedule_from_model_temporal_consistency(node_requests, availability_models, mu, num_intervals, incorporate_observation)
    else:
        raise ValueError(schedule_generation_method)

### Threshold simulated availabilities given "true" model
def generate_schedule_from_model_windows(node_requests, availability_models, mu, num_intervals):
    schedules = {}
    for request in node_requests:
        availability_model = availability_models[request]
        occupancies = []
        for i in range(num_intervals):
            occ = threshold_occupancy(availability_model[i])
            occupancies.append(occ)
        schedules[request] = occupancies
    return schedules

### Sample simulated availabilities given "true" model, incorporating temporal consistency
def sample_schedule_from_model_temporal_consistency(node_requests, availability_models, mu, num_intervals, temporal_consistency=True):
    schedules = {}
    for request in node_requests:
        availability_model = availability_models[request]
